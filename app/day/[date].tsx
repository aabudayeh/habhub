...
export default function DayDetail() {
  ...
  const dayEntries = state.entries.filter((entry) => entry.userId === state.currentUserId && entry.localDate === day);
  const dayPhotos = state.photos.filter((photo) => photo.userId === state.currentUserId && photo.localDate === day);
  ...
  const expandedRequested = [...new Set([
    ...requested.filter((id) => id !== TRACKED),
    ...(requested.includes(TRACKED) ? trackedSummary.metrics.map((metric) => metric.id) : []),
  ])];
  const initialIds = explicit ? expandedRequested.filter((id) => loggedIds.includes(id)) : loggedIds;
  ...
  const comparisonPhotos = compareDate.length ? state.photos.filter((photo) => photo.userId === state.currentUserId && photo.localDate === compareDate[0]) : [];
  const collage = [dayPhotos[0], comparisonPhotos[0]].filter(Boolean) as PhotoUpdate[];

  useEffect(() => setCompareDate(defaultPhotoDate ? [defaultPhotoDate] : []), [day, defaultPhotoDate]);

  function changeDay(next: string) {
    setDay(next);
    setCompareDate([]);
    const entries = state.entries.filter((entry) => entry.userId === state.currentUserId && entry.localDate === next);
    const ids = [...new Set(entries.map((entry) => entry.metricId))];
    if (state.photos.some((photo) => photo.userId === state.currentUserId && photo.localDate === next)) ids.push('progress_photo');
    setSelectedIds(ids);
  }

  function nearestWeight(photoDate: string) {
    const entries = state.entries
      .filter((entry) => entry.userId === state.currentUserId && entry.metricId === 'weight')
      .sort((a, b) => Math.abs(new Date(`${a.localDate}T12:00:00`).getTime() - new Date(`${photoDate}T12:00:00`).getTime()) - Math.abs(new Date(`${b.localDate}T12:00:00`).getTime() - new Date(`${photoDate}T12:00:00`).getTime()));
    return entries[0] ? `${Number(entries[0].value).toFixed(1)} kg${entries[0].localDate === photoDate ? '' : ' nearby'}` : 'No weight log';
  }
  ...
  async function saveCollage() {
    ...
    try {
      ...
      images.forEach((image, index) => {
        ...
        context.textAlign = 'center'; context.fillStyle = '#17211B'; context.font = 'bold 24px sans-serif'; context.fillText(collage[index].localDate, x + 270, 760);
        context.fillStyle = '#176B4D'; context.font = 'bold 19px sans-serif'; context.fillText(nearestWeight(collage[index].localDate), x + 270, 795);
      });
      canvas.toBlob((blob) => {
        if (!blob) return;
        const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `paceboard-${day}-comparison.png`; link.click(); URL.revokeObjectURL(url);
      }, 'image/png');
    } catch (error) {
  ...
  return <Screen keyboardShouldPersistTaps="handled">
    ...
    <Card style={styles.dateCard}>
      <View style={styles.dateNav}>
        ...
        <Pressable onPress={() => setCalendarOpen((value) => !value)} style={styles.dateButton}><Ionicons name="calendar-outline" size={17} color={palette.primary} /><Text style={styles.dateText}>{friendlyDate(day)}</Text><Ionicons name={calendarOpen ? 'chevron-up' : 'chevron-down'} size={16} color={palette.muted} /></Pressable>
        <Pressable onPress={() => changeDay(dateWithOffsetFrom(day, 1))} style={styles.arrow}><Ionicons name="chevron-forward" size={19} color={palette.ink} /></Pressable>
      </View>
    ...
    {dayPhotos.length ? <>
      ...
      <View style={styles.dayPhotos}>{dayPhotos.map((photo) => <Card key={photo.id} style={styles.photoCard}><ExpandableImage uri={photo.uri} caption={photo.caption} thumbnailStyle={styles.dayPhoto} /><Text style={styles.photoCaption}>{photo.caption || 'Progress photo'}</Text><Text style={styles.weight}>{photo.localDate} · {nearestWeight(photo.localDate)}</Text></Card>)}</View>
      {otherPhotoDates.length ? <><SectionHeader title="Compare with another date" /><MetricSelector items={otherPhotoDates.map((date) => ({ id: date, label: date, icon: 'calendar-outline', sublabel: `${state.photos.filter((photo) => photo.userId === state.currentUserId && photo.localDate === date).length} photo(s)` }))} selectedIds={compareDate} onChange={setCompareDate} multiple={false} title="Older photo date" /></> : null}
...
function TrackedCard({ state, day }: { state: AppState; day: string }) {
  ...
  return <Card style={styles.tracked}>
    <View style={styles.trackedTop}><Ionicons name="checkmark-done" size={22} color={palette.primary} /><View style={styles.grow}><Text style={styles.trackedTitle}>Tracked goals</Text><Text style={styles.meta}>{summary.met}/{summary.total} goals completed on this date</Text></View><Text style={styles.fraction}>{summary.met}/{summary.total}</Text></View>
    <View style={styles.goalChips}>{summary.metrics.map((metric) => {
      const reached = goalReached(metric, safeMetricValue(state, metric, state.currentUserId, day), effectiveGoalTarget(state, metric, state.currentUserId, day));
      return <Chip key={metric.id} label={`${metric.name}: ${reached ? 'met' : 'not met'}`} selected={reached} />;
    })}</View>
  </Card>;
}

function EntryRow({ entry, metric }: { entry: MetricEntry; metric: MetricDefinition }) {
  const label = typeof entry.value === 'string' ? entry.value : formatMetricValue(metric, entry.value === true ? 1 : entry.value === false ? 0 : Number(entry.value));
  return <View style={[styles.entry, { borderLeftColor: metric.color, backgroundColor: `${metric.color}0D` }]}>
    <View style={styles.grow}><View style={styles.entryMetric}><Ionicons name={metric.icon as keyof typeof Ionicons.glyphMap} size={13} color={metric.color} /><Text style={[styles.entryMetricText, { color: metric.color }]}>{metric.name}</Text></View><View style={styles.entryTop}><Text style={[styles.entryValue, { color: metric.color }]}>{label}</Text><Text style={styles.entryTime}>{new Date(entry.recordedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text></View>{entry.label ? <Text style={styles.entryLabel}>{entry.label}</Text> : null}{entry.note ? <Text style={styles.entryNote}>{entry.note}</Text> : null}{entry.nutrition ? <Text style={styles.nutrition}>{[['Protein', entry.nutrition.proteinG, 'g'], ['Fat', entry.nutrition.fatG, 'g'], ['Carbs', entry.nutrition.carbsG, 'g'], ['Fiber', entry.nutrition.fiberG, 'g'], ['Sodium', entry.nutrition.sodiumMg, 'mg']].filter((item) => item[1]).map((item) => `${item[0]} ${item[1]}${item[2]}`).join(' · ')}</Text> : null}</View>
    {entry.imageUri ? <ExpandableImage uri={entry.imageUri} thumbnailStyle={styles.entryImage} /> : null}
  </View>;
}

function AlignmentCard({ status }: { status: ReturnType<typeof deficitRealityCheckAtDate> }) {
  const copy = { aligned: 'Scale change roughly matches the reported deficit.', reported_ahead: 'Reported deficit is ahead of the scale trend; water shifts or missing intake may explain it.', scale_ahead: 'Scale change is ahead of the reported deficit; short-term water shifts are common.', noise: 'Weight change is within the normal fluctuation margin.', insufficient: 'A previous weight entry is needed for comparison.' }[status.status];
  return <><SectionHeader title="Reporting alignment" /><Card style={styles.alignment}><Ionicons name={status.status === 'aligned' ? 'checkmark-circle' : 'analytics-outline'} size={24} color={palette.primary} /><View style={styles.grow}><Text style={styles.alignmentTitle}>{status.status === 'aligned' ? 'Reported and scale trends align' : 'Estimate from this weight entry'}</Text><Text style={styles.entryNote}>{copy}</Text>{status.status !== 'insufficient' ? <Text style={styles.nutrition}>Reported {Math.round(status.reportedDailyDeficit)} kcal/day · scale estimate {Math.round(status.actualDailyDeficit)} kcal/day</Text> : null}</View></Card></>;
}
