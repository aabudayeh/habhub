...
export default function SignInScreen() {
  ...
  if (auth.status === 'signedIn' || auth.status === 'demo') return <Redirect href="/" />;
  ...
  return <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    <Screen keyboardShouldPersistTaps="handled" contentContainerStyle={styles.screen}>
      ...
      <Card style={styles.card}>
        ...
        {mode !== 'magic' ? <><Text style={styles.label}>Password</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry autoComplete={mode === 'sign-up' ? 'new-password' : 'current-password'} placeholder="At least 8 characters" placeholderTextColor={palette.faint} style={styles.input} /></> : null}
