...
import { Card, Chip, PageHeader, Screen, SectionHeader } from '@/src/components/ui';
import { dateKey, dateRangeEnding, dateWithOffsetFrom, friendlyDate, monthDateRange, shortDay } from '@/src/domain/date';
...
export default function Insights() {
  ...
  const dates = view === 'week' ? dateRangeEnding(weekAnchor, 7) : monthDateRange(month).filter((date) => date <= today);
  const selectorItems = [
    { id: TRACKED, label: 'Tracked goals', icon: 'checkmark-done-outline' as const, color: TRACKED_COLOR, sublabel: 'All goals enabled on Today' },
    ...metrics.map((metric) => ({ id: metric.id, label: metric.name, icon: metric.icon as keyof typeof Ionicons.glyphMap, color: metric.color })),
  ];
  ...
  function openDay(day: string) {
    router.push({ pathname: '/day/[date]' as never, params: { date: day, ...(filterTouched ? { metrics: selectedIds.join(',') } : {}) } });
  }
  ...
  function dayVisuals(day: string) {
    return rawDayVisuals(day).map((item) => ({ ...item, progress: Math.min(item.progress, 1) }));
  }
  ...
  function status(day: string) {
    ...
    return progress.every((value) => value >= 1) ? 'met' as const : progress.some((value) => value > 0) ? 'partial' as const : 'none' as const;
  }

  const legendItems = [
    ...(tracked ? [{ id: TRACKED, name: 'Tracked goals', color: TRACKED_COLOR }] : []),
    ...selectedMetrics.map((metric) => ({ id: metric.id, name: metric.name, color: metric.color })),
  ].slice(0, 5);
...
const styles = StyleSheet.create({
  recap: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: palette.ink, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 13 },
  recapText: { color: palette.white, fontSize: 11, fontWeight: '900' },
  today: { flexDirection: 'row', alignItems: 'center', gap: 11, backgroundColor: palette.card, borderWidth: 1, borderColor: palette.border, borderRadius: 19, padding: 14, marginBottom: 12 },
  copy: { flex: 1 },
  todayTitle: { color: palette.ink, fontSize: 13, fontWeight: '900' },
  todayText: { color: palette.muted, fontSize: 9, lineHeight: 14, marginTop: 2 },
  ...
  eyebrow: { color: palette.primary, fontSize: 9, fontWeight: '900', letterSpacing: 1 },
  cardTitle: { color: palette.ink, fontSize: 18, fontWeight: '900', marginTop: 2 },
  legend: { color: palette.muted, fontSize: 9 },
  legendWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: palette.card, borderWidth: 1, borderColor: palette.border, borderRadius: 10, paddingHorizontal: 8, paddingVertical: 6 },
  legendDot: { width: 8, height: 8, borderRadius: 4 },
  legendName: { color: palette.muted, fontSize: 9, fontWeight: '800' },
  hint: { color: palette.muted, fontSize: 9, lineHeight: 14, textAlign: 'center', marginTop: 10 },
  weekNav: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  arrow: { width: 36, height: 36, borderRadius: 12, backgroundColor: palette.canvas, alignItems: 'center', justifyContent: 'center' },
  navCopy: { alignItems: 'center' },
  weekChart: { position: 'relative', height: 210, flexDirection: 'row', alignItems: 'flex-end', gap: 6, marginTop: 16 },
  goalReference: { position: 'absolute', zIndex: 3, left: 0, right: 0, top: 46, borderTopWidth: 1, borderStyle: 'dashed', borderTopColor: palette.ink },
  goalReferenceLabel: { position: 'absolute', right: 2, top: -15, color: palette.ink, fontSize: 7, fontWeight: '900', backgroundColor: palette.card, paddingHorizontal: 4 },
  dayColumn: { flex: 1, alignItems: 'center', height: '100%' },
  bars: { height: 164, width: '100%', flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'center', gap: 2, borderBottomWidth: 1, borderBottomColor: palette.border },
  bar: { width: '15%', minWidth: 3, borderTopLeftRadius: 3, borderTopRightRadius: 3 },
  ...
  summaryLabel: { color: palette.muted, fontSize: 8, lineHeight: 13, marginTop: 2 },
  remaining: { color: palette.ink, fontSize: 8, lineHeight: 12, fontWeight: '700', marginTop: 5 },
  goalLine: { color: palette.primary, fontSize: 9, fontWeight: '800', marginTop: 6 },
  streakLine: { color: palette.muted, fontSize: 8, fontWeight: '800', marginTop: 4 },
});
