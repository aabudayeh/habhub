...
assert.match(screen, /optionsOpen=\{calendarOptionsOpen\}/, "Calendar options are controlled by the compact page header");
assert.match(screen, /label="Schedule view"/);
assert.doesNotMatch(screen, /subtitle="Your shared week/, "The calendar must not be pushed down by a promotional subtitle");
assert.match(screen, /optionsFooter=\{[\s\S]*Group event reminders/, "Reminder opt-in remains available in calendar options");
...
assert.match(calendar, /visible=\{optionsOpen\}/, "View and filter controls live in an accessible, initially closed modal");
assert.match(calendar, /\{actions\}/, "Creation actions remain adjacent to the date navigation and calendar grid");
