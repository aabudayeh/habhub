...
function metricFromRow(row: Record<string, any>): MetricDefinition {
  ...
  return {
    ...
    sections: configuration.sections ?? { today: true, group: true, insights: true },
    order: Number(configuration.order ?? 0),
    activeFrom: configuration.activeFrom ?? new Date().toISOString().slice(0, 10),
  };
}
...
async function signedUrls(paths: string[]) {
  ...
  const { data, error } = await client.storage.from(MEDIA_BUCKET).createSignedUrls(unique, 60 * 60);
  if (error) throw error;
  const pairs: [string, string][] = [];
  for (const item of data ?? []) if (item.path && item.signedUrl) pairs.push([item.path, item.signedUrl]);
  return new Map<string, string>(pairs);
}

async function upsertMetrics(groupId: string, metrics: MetricDefinition[]) {
  const client = requireCloud();
  const { data: existing, error: existingError } = await client.from('metric_definitions').select('id, slug').eq('group_id', groupId);
  if (existingError) throw existingError;
  const removed = (existing ?? []).filter((row) => !metrics.some((metric) => metric.id === row.slug)).map((row) => row.id);
  if (removed.length) {
    const { error } = await client.from('metric_definitions').delete().in('id', removed);
    if (error) throw error;
  }
  if (metrics.length) {
    const { error } = await client.from('metric_definitions').upsert(metrics.map((metric) => metricRow(groupId, metric)), { onConflict: 'group_id,owner_user_id,slug' });
    if (error) throw error;
  }
}
...
async function groupMembers(groupIds: string[]) {
  ...
  const { data: membership, error } = await client.from('group_members').select('group_id, user_id, role').in('group_id', groupIds);
  if (error) throw error;
  const userIds = [...new Set((membership ?? []).map((row) => row.user_id))];
  const { data: profiles, error: profileError } = userIds.length
    ? await client.from('profiles').select('id, display_name, avatar_path').in('id', userIds)
    : { data: [], error: null };
  if (profileError) throw profileError;
  const profileMap = new Map((profiles ?? []).map((profile) => [profile.id, profile]));
  const urls = await signedUrls((profiles ?? []).map((profile) => profile.avatar_path).filter(Boolean));
  ...
  for (const membershipRow of membership ?? []) {
    ...
    const member: Member = {
      ...
      avatarUri: profile?.avatar_path ? urls.get(profile.avatar_path) ?? undefined : undefined,
    };
    result.set(membershipRow.group_id, [...(result.get(membershipRow.group_id) ?? []), member]);
  }
  return result;
}
...
export async function loadCloudGroupShells(): Promise<Group[]> {
  const client = requireCloud();
  const { data: memberships, error } = await client.from('group_members').select('group_id, role');
  if (error) throw error;
  const groupIds = (memberships ?? []).map((row) => row.group_id);
  if (!groupIds.length) return [];
  const [{ data: rows, error: groupError }, { data: metrics, error: metricError }, members] = await Promise.all([
    client.from('groups').select('id, name, invite_code, template_name, settings').in('id', groupIds),
    client.from('metric_definitions').select('*').in('group_id', groupIds),
    groupMembers(groupIds),
  ]);
  if (groupError) throw groupError;
  if (metricError) throw metricError;
  return (rows ?? []).map((row): Group => ({
    id: row.id,
    name: row.name,
...
export async function createCloudGroup(name: string, metrics: MetricDefinition[], user: User, displayName?: string) {
  const client = requireCloud();
  if (displayName?.trim()) {
  ...
  if (!created) throw new Error('Could not create a unique invitation code. Try again.');
...
export async function joinCloudGroup(code: string) {
  const client = requireCloud();
  const { data, error } = await client.rpc('join_group_with_code', { code: code.trim().toUpperCase() });
  if (error) throw error;
...
export async function leaveCloudGroup(groupId: string) {
  ...
  const [{ data: group, error: groupError }, { data: members, error: memberError }] = await Promise.all([
    ...
    client.from('group_members').select('user_id, role, joined_at').eq('group_id', groupId).order('joined_at'),
  ]);
  if (groupError) throw groupError;
  if (memberError) throw memberError;
  if (group.owner_id === userData.user.id) {
    const successor = (members ?? []).filter((member) => member.user_id !== userData.user!.id).sort((a, b) => (a.role === 'admin' ? -1 : 1) - (b.role === 'admin' ? -1 : 1))[0];
    if (!successor) {
      const { error } = await client.from('groups').delete().eq('id', groupId);
      if (error) throw error;
      return;
    }
    const { error: ownerError } = await client.from('groups').update({ owner_id: successor.user_id }).eq('id', groupId);
    if (ownerError) throw ownerError;
    const { error: roleError } = await client.from('group_members').update({ role: 'owner' }).eq('group_id', groupId).eq('user_id', successor.user_id);
    if (roleError) throw roleError;
  }
  const { error } = await client.from('group_members').delete().eq('group_id', groupId);
  if (error) throw error;
}

export async function loadCloudWorkspace(state: AppState, groupId: string): Promise<AppState> {
  const client = requireCloud();
  const shells = await loadCloudGroupShells();
  const group = shells.find((candidate) => candidate.id === groupId);
  if (!group) throw new Error('This group is unavailable or you no longer have access.');
  const personalMetricById = new Map(state.metrics.map((metric) => [metric.id, metric]));
  const metrics = (group.metricConfiguration ?? []).map((metric) => {
    const personal = personalMetricById.get(metric.id);
    return personal ? {
      ...metric,
      goal: personal.goal,
  ...
  group.metricConfiguration = metrics;
  const { data: metricRows, error: metricError } = await client.from('metric_definitions').select('id, slug').eq('group_id', groupId);
  if (metricError) throw metricError;
  const metricIds = (metricRows ?? []).map((row) => row.id);
  const slugById = new Map((metricRows ?? []).map((row) => [row.id, row.slug]));
  const [entryResult, statusResult, messageResult, photoResult] = await Promise.all([
    metricIds.length ? client.from('metric_entries').select('*').in('metric_id', metricIds).order('recorded_at') : Promise.resolve({ data: [], error: null }),
    client.from('daily_metric_status').select('*').eq('group_id', groupId),
    client.from('messages').select('*').eq('group_id', groupId).order('created_at'),
    client.from('photo_updates').select('*').eq('group_id', groupId).order('created_at', { ascending: false }),
  ]);
  if (entryResult.error) throw entryResult.error;
  if (statusResult.error) throw statusResult.error;
  if (messageResult.error) throw messageResult.error;
  if (photoResult.error) throw photoResult.error;
  const mediaIds = (photoResult.data ?? []).map((photo) => photo.media_asset_id);
  const { data: media, error: mediaError } = mediaIds.length
    ? await client.from('media_assets').select('id, storage_path, captured_at').in('id', mediaIds)
    : { data: [], error: null };
  if (mediaError) throw mediaError;
  const mediaById = new Map((media ?? []).map((item) => [item.id, item]));
  const paths = [
    ...(entryResult.data ?? []).map((entry) => entry.image_path).filter(Boolean),
    ...(messageResult.data ?? []).map((message) => message.image_path).filter(Boolean),
  ...
  const entries: MetricEntry[] = (entryResult.data ?? []).map((entry) => ({
    ...
    imageUri: entry.image_path ? urls.get(entry.image_path) ?? undefined : undefined,
  }));
  const dailyMetricStatuses: DailyMetricStatus[] = (statusResult.data ?? []).map((status) => ({
    groupId,
  ...
  return {
    ...
    messages,
    dailyMetricStatuses,
    selectedGroupMetricId: metrics.some((metric) => metric.id === state.selectedGroupMetricId) ? state.selectedGroupMetricId : metrics[0]?.id ?? 'steps',
  };
}
...
export async function pushCloudWorkspace(state: AppState) {
  ...
  if (profileError) throw profileError;
  const profile = state.energyProfiles[state.currentUserId] ?? state.settings.energyProfile;
  ...
  if (canManage) {
    ...
    if (current.role === 'owner') {
      for (const member of state.group.members.filter((member) => member.role !== 'owner')) {
        const { error } = await client.from('group_members').update({ role: member.role }).eq('group_id', state.group.id).eq('user_id', member.id);
        if (error) throw error;
      }
    }
  }
  const { data: metricRows, error: metricError } = await client.from('metric_definitions').select('id, slug').eq('group_id', state.group.id);
  if (metricError) throw metricError;
  ...
  const statusDates = [...new Set(ownedEntries.map((entry) => entry.localDate))];
  await client.from('daily_metric_status').delete().eq('group_id', state.group.id).eq('user_id', state.currentUserId);
  const statuses = statusDates.flatMap((localDate) => state.metrics.filter((metric) => metric.dataType !== 'text' && idBySlug.has(metric.id)).map((metric) => {
    ...
    return {
      ...
      goal_reached: goalReached(metric, value, effectiveGoalTarget(state, metric, state.currentUserId, localDate)),
      score_contribution: Math.min(goalProgress(metric, value, effectiveGoalTarget(state, metric, state.currentUserId, localDate)), 1) * 100,
    };
  }));
  if (statuses.length) {
  ...
  const deletedPhotoIds = (oldPhotos ?? []).map((photo) => photo.client_generated_id).filter((id) => id && !currentPhotoIds.has(id));
  if (deletedPhotoIds.length) await client.from('photo_updates').delete().eq('group_id', state.group.id).eq('owner_user_id', state.currentUserId).in('client_generated_id', deletedPhotoIds);
  for (const photo of ownedPhotos) {
    const { data: asset, error: assetError } = await client.from('media_assets').upsert({
  ...
  await client.from('group_member_aliases').delete().eq('owner_user_id', state.currentUserId).eq('group_id', state.group.id);
  const aliasRows = Object.entries(aliases).filter(([, alias]) => alias.trim()).map(([memberId, alias]) => ({ owner_user_id: state.currentUserId, group_id: state.group.id, subject_user_id: memberId, alias: alias.trim() }));
  if (aliasRows.length) {
    const { error } = await client.from('group_member_aliases').insert(aliasRows);
    if (error) throw error;
  }
}
