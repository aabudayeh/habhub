...
export function MonthCalendar({ selectedDate, onSelect, hasActivity, dayStatus, dayVisuals, monthDate, onMonthChange }: { selectedDate: string; onSelect: (date: string) => void; hasActivity?: (date: string) => boolean; dayStatus?: (date: string) => 'met' | 'partial' | 'none'; dayVisuals?: (date: string) => { color: string; progress: number; goalReached?: boolean }[]; monthDate?: string; onMonthChange?: (date: string) => void }) {
  ...
  return <View {...swipe.panHandlers}>
    <View style={styles.heading}><Pressable onPress={()=>shift(-1)} style={[styles.arrow,{backgroundColor:colors.canvas}]}><Ionicons name="chevron-back" size={19} color={colors.ink}/></Pressable><Text style={[styles.title,{color:colors.ink}]}>{title}</Text><Pressable onPress={()=>shift(1)} style={[styles.arrow,{backgroundColor:colors.canvas}]}><Ionicons name="chevron-forward" size={19} color={colors.ink}/></Pressable></View>
    <View style={styles.grid}>{['S','M','T','W','T','F','S'].map((day,index)=><Text key={`${day}-${index}`} style={[styles.weekday,{color:colors.faint}]}>{day}</Text>)}{days.map((day)=>{const selected=day.key===selectedDate;const status=dayStatus?.(day.key);const visuals=dayVisuals?.(day.key)??[];const allTrackedMet=visuals.length>0&&visuals[0].color==='#9B6BDB'&&visuals[0].goalReached;return <Pressable key={day.key} onPress={()=>onSelect(day.key)} style={[styles.day,status==='met'&&styles.metDay,status==='partial'&&{backgroundColor:colors.primarySoft},selected&&styles.selected]}><Text style={[styles.dayText,{color:colors.ink},!day.current&&{color:colors.faint},selected&&styles.selectedText]}>{day.number}</Text>{visuals.length?<View style={styles.visuals}>{visuals.slice(0,5).map((visual,index)=><View key={index} style={[styles.visualTrack,{backgroundColor:colors.border}]}><View style={[styles.visualFill,{backgroundColor:visual.color,width:`${Math.min(Math.max(visual.progress,0),1)*100}%`}]}/>{visual.goalReached?<View style={[styles.goalTick,{borderColor:colors.card}]}/>:null}</View>)}</View>:hasActivity?.(day.key)||status&&status!=='none'?<View style={[styles.dot,status==='met'&&styles.metDot,selected&&styles.dotSelected]}/>:null}{allTrackedMet?<Ionicons name="checkmark-circle" size={12} color="#9B6BDB" style={styles.goalBadge}/>:null}</Pressable>;})}</View>
  </View>;
}
const styles=StyleSheet.create({
  heading:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginBottom:10},title:{color:palette.ink,fontSize:15,fontWeight:'900'},arrow:{width:36,height:36,borderRadius:12,backgroundColor:palette.canvas,alignItems:'center',justifyContent:'center'},grid:{flexDirection:'row',flexWrap:'wrap'},weekday:{width:'14.285%',textAlign:'center',color:palette.faint,fontSize:9,fontWeight:'900',paddingVertical:7},day:{width:'14.285%',aspectRatio:1,minHeight:50,alignItems:'center',justifyContent:'flex-start',paddingTop:5,borderRadius:12},selected:{backgroundColor:palette.primary},dayText:{color:palette.ink,fontSize:12,fontWeight:'800',zIndex:1},muted:{color:'#C1C9C3'},selectedText:{color:palette.white},dot:{position:'absolute',bottom:5,width:4,height:4,borderRadius:2,backgroundColor:palette.primary},dotSelected:{backgroundColor:palette.lime},
  ...
  goalTick:{position:'absolute',top:-5,right:-3,width:8,height:8,borderRadius:4,backgroundColor:'#D8A126',borderWidth:1.5,borderColor:palette.card},
