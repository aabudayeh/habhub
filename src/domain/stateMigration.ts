...
function activeEnergyMetricList(
  metrics: MetricDefinition[] | undefined,
  defaults: AppState,
) {
  if (!metrics) return metrics;
  const workoutCalories = metrics.find(
    (metric) => metric.id === WORKOUT_CALORIES_METRIC_ID,
  );
  const filtered = metrics.filter(
    (metric) => !RETIRED_METRIC_IDS.has(metric.id),
  );
  if (!workoutCalories || filtered.some((metric) => metric.id === "exercise"))
    return filtered;
  const preset = defaults.metrics.find((metric) => metric.id === "exercise");
  if (!preset) return filtered;
  return [
    ...filtered,
    {
      ...preset,
      order: workoutCalories.order,
      activeFrom: workoutCalories.activeFrom,
      defaultVisibility: workoutCalories.defaultVisibility,
      sections: workoutCalories.sections,
    },
  ];
}

/**
 * Retire the duplicate Workout calories tracker only after preserving every
 * calorie row in canonical Active energy. Entry visibility and provenance are
 * copied exactly, while the old owner rows receive durable cloud tombstones.
 */
export function retireWorkoutCalories(
  state: AppState,
  defaults: AppState,
): AppState {
  const migrated = migrateRetiredWorkoutCaloriesEntries(
    state.entries,
    state.currentUserId,
    [
  ...
  const removedOwnEntryIds = migrated.removedOwnEntryIds;
  const removedOwnEntryIdSet = new Set(removedOwnEntryIds);
  const groups = state.groups.map((group) => ({
    ...group,
    metricConfiguration: activeEnergyMetricList(
  ...
  return {
    ...state,
    metrics: activeEnergyMetricList(state.metrics, defaults) ?? state.metrics,
    ...
    settings: {
      ...state.settings,
      // Keep this true so an older client that understands only the temporary
      // restoration marker does not re-add the retired tracker from defaults.
      ...
      googleHealthEntryOverrides: Object.fromEntries(
        ...
        ).filter(([entryId]) => !removedOwnEntryIdSet.has(entryId)),
      ),
