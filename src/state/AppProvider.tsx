...
type Action =
  ...
  | { type: 'updateMetric'; metricId: string; changes: Partial<MetricDefinition> }
  | { type: 'deleteMetric'; metricId: string }
  | { type: 'setMetricSection'; metricId: string; section: DashboardSection; value: boolean; historyMode?: 'today' | 'history' }
  ...
  | { type: 'addPhoto'; uri: string; caption: string; visibility: Visibility; localDate?: string; capturedAt?: string }
  | { type: 'setPhotoVisibility'; photoId: string; visibility: Visibility }
  | { type: 'sendMessage'; text: string; conversationId: string; recipientId?: string; imageUri?: string }
  ...
  | { type: 'importHealth'; entries: MetricEntry[]; provider: NonNullable<MetricEntry['sourceProvider']>; metricIds: string[]; fromDate: string }
  | { type: 'reset' };
...
function withActiveMetrics(state: AppState, metrics: MetricDefinition[]): AppState {
  const group = { ...state.group, metricConfiguration: metrics };
  const groups = (state.groups?.length ? state.groups : [state.group]).map((candidate) =>
    candidate.id === group.id ? group : candidate,
  );
  return { ...state, group, groups, metrics };
}
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'log': {
      ...
      const metric = state.metrics.find((candidate) => candidate.id === action.metricId);
      const previousValue = metric ? safeMetricValue(state, metric, state.currentUserId, localDate) : 0;
      ...
      if (
        ...
        !goalReached(metric, previousValue, effectiveGoalTarget(state, metric, state.currentUserId, localDate)) &&
        goalReached(metric, safeMetricValue(nextState, metric, state.currentUserId, localDate), effectiveGoalTarget(nextState, metric, state.currentUserId, localDate))
      ) {
        nextState.messages = [
          ...nextState.messages,
          {
            id: uniqueId('auto'), senderId: 'system', conversationId: `group:${state.group.id}`, kind: 'cheer',
        ...
        state.metrics.map((metric) =>
          metric.id === action.metricId
            ? {
                ...metric,
                ...action.changes,
                activeFrom: action.changes.scoreWeight !== undefined && action.changes.scoreWeight > 0 && metric.scoreWeight <= 0
                  ? dateKey()
                  : metric.activeFrom,
              }
            : metric,
    ...
    case 'deleteMetric':
      return {
        ...withActiveMetrics(state, state.metrics.filter((metric) => metric.id !== action.metricId).map((metric, order) => ({ ...metric, order }))),
        entries: state.entries.filter((entry) => entry.metricId !== action.metricId),
        trackedGoalPeriods: Object.fromEntries(Object.entries(state.trackedGoalPeriods).filter(([metricId]) => metricId !== action.metricId)),
        selectedGroupMetricId: state.selectedGroupMetricId === action.metricId ? 'steps' : state.selectedGroupMetricId,
      };
    case 'setMetricSection': {
      const metric = state.metrics.find((candidate) => candidate.id === action.metricId);
      if (!metric) return state;
      const next = withActiveMetrics(
        state,
        state.metrics.map((metric) =>
          metric.id === action.metricId
            ? {
                ...metric,
                sections: { ...metric.sections, [action.section]: action.value },
              }
            : metric,
        ),
      );
      if (action.section !== 'today') return next;
      const existing = state.trackedGoalPeriods?.[metric.id] ?? (metric.sections.today ? [{ from: metric.activeFrom }] : []);
      if (action.value) {
      ...
      const periods = existing.flatMap((period) => !period.to
        ? period.from <= yesterday ? [{ ...period, to: yesterday }] : []
        : [period]);
      return { ...next, trackedGoalPeriods: { ...state.trackedGoalPeriods, [metric.id]: periods } };
    }
    case 'moveMetric': {
      const ordered = [...state.metrics].sort((a, b) => a.order - b.order);
      const index = ordered.findIndex((metric) => metric.id === action.metricId);
      const nextIndex = index + action.direction;
      if (index < 0 || nextIndex < 0 || nextIndex >= ordered.length) return state;
      [ordered[index], ordered[nextIndex]] = [ordered[nextIndex], ordered[index]];
      return withActiveMetrics(state, ordered.map((metric, order) => ({ ...metric, order })));
    }
    case 'reorderMetric': {
      const ordered = [...state.metrics].sort((a, b) => a.order - b.order);
      const index = ordered.findIndex((metric) => metric.id === action.metricId);
      ...
      ordered.splice(Math.max(0, Math.min(action.targetIndex, ordered.length)), 0, moved);
      return withActiveMetrics(state, ordered.map((metric, order) => ({ ...metric, order })));
    }
    ...
    case 'setPhotoVisibility':
      return {
        ...
        photos: state.photos.map((photo) =>
          photo.id === action.photoId ? { ...photo, visibility: action.visibility } : photo,
        ),
      };
    case 'sendMessage':
      return {
    ...
    case 'createGroup': {
      ...
      const group: Group = {
        ...
        metricConfiguration: state.metrics.map((metric) => ({ ...metric })),
      };
      const groups = state.groups.map((candidate) => candidate.id === state.group.id ? { ...candidate, metricConfiguration: state.metrics } : candidate);
      return { ...state, group, groups: [...groups, group], metrics: group.metricConfiguration ?? state.metrics };
    }
    case 'joinGroup': {
      const normalized = action.code.trim().toUpperCase();
      if (!normalized) return state;
      const existing = state.groups.find((group) => group.inviteCode.toUpperCase() === normalized);
    ...
    case 'switchGroup': {
      ...
      const groups = state.groups.map((candidate) => candidate.id === state.group.id ? { ...candidate, metricConfiguration: state.metrics } : candidate);
      return { ...state, group, groups, metrics: group.metricConfiguration ?? state.metrics };
    }
    case 'leaveGroup': {
      ...
      return { ...state, groups, group, metrics: group.metricConfiguration ?? state.metrics };
    }
    case 'nickname': {
      const groupAliases = state.settings.memberNicknamesByGroup?.[state.group.id] ?? {};
      return { ...state, settings: { ...state.settings, memberNicknamesByGroup: { ...state.settings.memberNicknamesByGroup, [state.group.id]: { ...groupAliases, [action.memberId]: action.nickname.trim() } } } };
    }
    case 'groupRestDays': {
    ...
    case 'memberName': {
      ...
      const updateMembers = (group: Group): Group => ({ ...group, members: group.members.map((member) => member.id === action.memberId ? { ...member, name, initials: name.split(/\s+/).slice(0, 2).map((part) => part[0]).join('').toUpperCase() } : member) });
      const groups = state.groups.map(updateMembers);
      return { ...state, groups, group: updateMembers(state.group) };
    }
    case 'memberRole': {
      const current = state.group.members.find((member) => member.id === state.currentUserId);
      const target = state.group.members.find((member) => member.id === action.memberId);
      if (current?.role !== 'owner' || !target || target.role === 'owner') return state;
      const group = { ...state.group, members: state.group.members.map((member) => member.id === action.memberId ? { ...member, role: action.role } : member) };
      return { ...state, group, groups: state.groups.map((candidate) => candidate.id === group.id ? group : candidate) };
    }
    case 'importHealth': {
      const replaceIds = new Set(action.metricIds);
      const preserved = state.entries.filter((entry) => !(
        entry.userId === state.currentUserId &&
...
type AppContextValue = {
  ...
  setMetricSection: (metricId: string, section: DashboardSection, value: boolean, historyMode?: 'today' | 'history') => void;
  ...
  addPhoto: (uri: string, caption: string, visibility: Visibility, localDate?: string, capturedAt?: string) => void;
  setPhotoVisibility: (photoId: string, visibility: Visibility) => void;
  sendMessage: (text: string, conversationId?: string, recipientId?: string, imageUri?: string) => void;
  ...
  importHealthEntries: (entries: MetricEntry[], provider: NonNullable<MetricEntry['sourceProvider']>, metricIds: string[], fromDate: string) => void;
...
export function AppProvider({ children }: PropsWithChildren) {
    ...
    AsyncStorage.getItem(APP_STORAGE_KEY)
      .then((saved) => {
        if (saved) {
          ...
          const isDefaultDemo = (restored.group?.id ?? defaults.group.id) === defaults.group.id;
          let restoredMetrics = restoredVersion < 3
            ? [
                ...(restored.metrics ?? []),
                ...defaults.metrics.filter((candidate) => !(restored.metrics ?? []).some((metric) => metric.id === candidate.id)),
              ].map((metric, order) => ({ ...metric, activeFrom: metric.activeFrom ?? dateKey(), order }))
            : (restored.metrics ?? defaults.metrics).map((metric) => ({ ...metric, activeFrom: metric.activeFrom ?? dateKey() }));
          if (restoredVersion < 9 && isDefaultDemo) {
            const savedById = new Map(restoredMetrics.map((metric) => [metric.id, metric]));
            const builtInIds = new Set(defaults.metrics.map((metric) => metric.id));
            restoredMetrics = [
              ...defaults.metrics.map((metric) => ({
                ...metric,
                ...(savedById.get(metric.id) ?? {}),
                activeFrom: metric.activeFrom,
                rankingDirection: metric.id === 'deficit' ? 'closest' as const : (savedById.get(metric.id)?.rankingDirection ?? metric.rankingDirection),
              })),
          ...
          const migratedTrackedGoals = restoredVersion < 9 && isDefaultDemo
            ? Object.fromEntries(restoredMetrics.map((metric) => [metric.id, defaults.trackedGoalPeriods[metric.id] ?? restored.trackedGoalPeriods?.[metric.id] ?? []]))
          ...
          const restoredState: AppState = {
            ...
            settings: {
              ...
              healthSync: {
                ...
                dataTypes: { ...defaults.settings.healthSync.dataTypes, ...restored.settings?.healthSync?.dataTypes },
              },
              memberNicknamesByGroup: restored.settings?.memberNicknamesByGroup ?? {
                [restored.group?.id ?? defaults.group.id]: { ...(restored.settings?.memberNicknames ?? {}) },
              },
              badgeShowcaseByGroup: { ...defaults.settings.badgeShowcaseByGroup, ...restored.settings?.badgeShowcaseByGroup },
              notifications: { ...defaults.settings.notifications, ...restored.settings?.notifications },
            },
            trackedGoalPeriods: migratedTrackedGoals,
            entries: restoredVersion < 9 && isDefaultDemo
              ? [...(restored.entries ?? []).filter((entry) => !defaultEntryIds.has(entry.id)), ...defaults.entries]
              : (restored.entries ?? defaults.entries),
            photos: restoredVersion < 9 && isDefaultDemo
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      AsyncStorage.setItem(APP_STORAGE_KEY, JSON.stringify({ ...state, lastSavedAt: new Date().toISOString() })).catch(
        () => undefined,
      );
    }, 250);
  ...
  const value = useMemo<AppContextValue>(
    () => ({
      ...
      logMetric: (metricId, entryValue, visibility, mode = 'add', details) =>
        dispatch({ type: 'log', metricId, value: entryValue, visibility, mode, details }),
      addMetric: (metric) => dispatch({ type: 'addMetric', metric }),
      updateMetric: (metricId, changes) => dispatch({ type: 'updateMetric', metricId, changes }),
      deleteMetric: (metricId) => dispatch({ type: 'deleteMetric', metricId }),
      setMetricSection: (metricId, section, value, historyMode) => dispatch({ type: 'setMetricSection', metricId, section, value, historyMode }),
      moveMetric: (metricId, direction) => dispatch({ type: 'moveMetric', metricId, direction }),
      reorderMetric: (metricId, targetIndex) => dispatch({ type: 'reorderMetric', metricId, targetIndex }),
      selectGroupMetric: (metricId) => dispatch({ type: 'selectGroupMetric', metricId }),
      addPhoto: (uri, caption, visibility, localDate, capturedAt) => dispatch({ type: 'addPhoto', uri, caption, visibility, localDate, capturedAt }),
      setPhotoVisibility: (photoId, visibility) => dispatch({ type: 'setPhotoVisibility', photoId, visibility }),
      sendMessage: (text, conversationId = 'group', recipientId, imageUri) =>
        dispatch({ type: 'sendMessage', text, conversationId, recipientId, imageUri }),
      updateSettings: (changes) => dispatch({ type: 'settings', changes }),
      updateEnergyProfile: (changes) => dispatch({ type: 'energyProfile', changes }),
...
const styles = StyleSheet.create({
  ...
  loadingTitle: { color: palette.ink, fontSize: 24, fontWeight: '900', letterSpacing: -0.6 },
